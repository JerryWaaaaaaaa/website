#!/usr/bin/env node
// Generate a handoff entry for a range of commits (one push) and write it
// to a new file under handoffs/.
//
// Usage:
...
const HANDOFFS_DIR = resolve(REPO_ROOT, "handoffs");
...
function buildPrompt({ commits, stat, diff, range, baseShort, tipShort, now }) {
  ...
  const headerTitle = `## ${nowStamp(now)} — push ${baseShort}..${tipShort} (${commits.length} commit${commits.length === 1 ? "" : "s"})`;
...
function writeHandoffFile(entry, { stamp, baseShort, tipShort }) {
  mkdirSync(HANDOFFS_DIR, { recursive: true });
  const filename = `${stamp}-${baseShort}-${tipShort}.md`;
  const fullPath = resolve(HANDOFFS_DIR, filename);
  writeFileSync(fullPath, entry.trim() + "\n");
  return { fullPath, relPath: `handoffs/${filename}` };
}
...
function commitHandoff(commits, relPath) {
  git(["add", relPath]);
  const staged = git(["diff", "--cached", "--name-only"]);
  if (!staged.includes(relPath)) {
    log(`${relPath} had no staged changes; skipping commit.`);
    return;
  }
...
async function main() {
  ...
  const { relPath } = writeHandoffFile(entry, { stamp, baseShort, tipShort });
  log(`Wrote entry (${entry.length} chars) to ${relPath}`);

  commitHandoff(commits, relPath);
  log("Done.");
}
